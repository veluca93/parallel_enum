...
public:
	...
	bool ListRoots(const NodeCallback& cb) override {
		...
		for (node_t i = 0; i < graph_->size(); i++) {
			if (graph_->degree(i) == graph_->fwd_degree(i)) {
				...
				//call the callback for each root
				if (!cb(root)) {
					return false;
				}
			}
	...
	bool ListChildren(const CliqueEnumerationNode<node_t>& clique_info,
		...
		bool ret =
				Candidates(clique, parind, [this, max_bad, /*&bad_bitset_,*/&clique, &cb](node_t cand) {
			return ChildrenCand(
						clique, cand,
						[this, max_bad, /*&bad_bitset_, */cand, &clique, &cb](std::vector<node_t>* child) {
...
private:
	...
	bool Candidates(const std::vector<node_t>& clique, node_t parind,
		...
		//attempt
//		std::vector<bool> candidates_bitset_;
//		candidates_bitset_.resize(graph_->size());
//		std::vector<node_t> candidates_;
		for (node_t node : clique) {
